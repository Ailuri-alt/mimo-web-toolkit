...
Следовать поэтапному плану в `docs/ROADMAP.md` (22 этапа). Каждый этап завершается полностью рабочим состоянием. Никогда не пропускать этапы. Перед каждым этапом: объяснить архитектуру, показать дерево файлов, затем писать полный код.
